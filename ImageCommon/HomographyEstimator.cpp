...
{
	bool RANSACHomography::calErr(const Eigen::Matrix3f& mdl, const int& data, float& err)
	{
        const double& H_00 = mdl(0, 0);
        const double& H_01 = mdl(0, 1);
        const double& H_02 = mdl(0, 2);
        const double& H_10 = mdl(1, 0);
        const double& H_11 = mdl(1, 1);
        const double& H_12 = mdl(1, 2);
        const double& H_20 = mdl(2, 0);
        const double& H_21 = mdl(2, 1);
        const double& H_22 = mdl(2, 2);

        const double s_0 = m_kps1->at(data).pt.x;
        const double s_1 = m_kps1->at(data).pt.y;
        const double d_0 = m_kps2->at(data).pt.x;
        const double d_1 = m_kps2->at(data).pt.y;
        ...
        err = dd_0 * dd_0 + dd_1 * dd_1;
        if (err < m_errTh)
            return true;
		return false;
	}
	bool RANSACHomography::calMdl(const std::vector<const int*>& samples, Eigen::Matrix3f& mdl)
	{
        ...
        for (size_t i = 0; i < num_points; ++i) {
            p1(0) = m_kps1->at(*samples[i]).pt.x;
            p1(1) = m_kps1->at(*samples[i]).pt.y;
            p2(0) = m_kps2->at(*samples[i]).pt.x;
            p2(1) = m_kps2->at(*samples[i]).pt.y;
